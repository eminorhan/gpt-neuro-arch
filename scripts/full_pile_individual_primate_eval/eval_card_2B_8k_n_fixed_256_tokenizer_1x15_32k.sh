...
srun torchrun --nnodes $SLURM_NNODES --nproc_per_node 4 --max_restarts 1 --node_rank $SLURM_NODEID --rdzv_id 101 --rdzv_backend c10d --rdzv_endpoint "$MASTER_ADDR:$MASTER_PORT" ./evaluate.py --config ${CONFIG_FILE} --ckpt ${CKPT_PATH} --eval_steps 100 --eval_dirname "eval-card" --source_dataset "card"
