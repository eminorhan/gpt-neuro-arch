...
#SBATCH --time=6:00:00
#SBATCH --job-name=eval_full_pile_vcn_packed
#SBATCH --output=eval_full_pile_vcn_packed_%j.out   # %j gets replaced with the Job ID
...
for i in "${!CHECKPOINTS[@]}"; do
    ...
    echo "Starting vcn eval for $CKPT_PATH on nodes $SUB_NODELIST (Master: $MASTER_ADDR:$MASTER_PORT)"
    ...
    srun --nodelist=${SUB_NODELIST} \
         ...
         bash -c "torchrun --nnodes ${NODES_PER_JOB} \
                  ...
                  --ckpt ${CKPT_PATH} \
                  --eval_steps 1000 \
                  --eval_dirname \"eval-vcn\" \
                  --source_dataset \"vcn\"" \
         > "eval_vcn_log_${i}.out" 2>&1 &
...
wait
echo "All vcn evaluations complete."