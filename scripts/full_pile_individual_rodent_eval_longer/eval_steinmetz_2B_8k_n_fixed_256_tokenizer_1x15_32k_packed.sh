...
#SBATCH --nodes=30                  # Request all 30 nodes at once
#SBATCH --cpus-per-task=288
#SBATCH --ntasks-per-node=1
#SBATCH --gpus-per-node=4
#SBATCH --time=6:00:00
#SBATCH --job-name=eval_full_pile_steinmetz_packed
#SBATCH --output=eval_full_pile_steinmetz_packed_%j.out   # %j gets replaced with the Job ID
...
for i in "${!CHECKPOINTS[@]}"; do
    ...
    echo "Starting steinmetz eval for $CKPT_PATH on nodes $SUB_NODELIST (Master: $MASTER_ADDR:$MASTER_PORT)"
    ...
    srun --nodelist=${SUB_NODELIST} \
         ...
         bash -c "torchrun --nnodes ${NODES_PER_JOB} \
                  ...
                  --ckpt ${CKPT_PATH} \
                  --eval_steps 1000 \
                  --eval_dirname \"eval-steinmetz\" \
                  --source_dataset \"steinmetz\"" \
         > "eval_steinmetz_log_${i}.out" 2>&1 &
...
wait
echo "All steinmetz evaluations complete."