...
#SBATCH --time=6:00:00
#SBATCH --job-name=eval_full_pile_ibl_packed
#SBATCH --output=eval_full_pile_ibl_packed_%j.out   # %j gets replaced with the Job ID
...
for i in "${!CHECKPOINTS[@]}"; do
    ...
    echo "Starting eval for $CKPT_PATH on nodes $SUB_NODELIST (Master: $MASTER_ADDR:$MASTER_PORT)"
    
    # Launch srun in the background for this node subset
    # Notice the \ before $SLURM_NODEID so it evaluates on the target node, not locally!
