...
#SBATCH --time=6:00:00
#SBATCH --job-name=eval_full_pile_steinmetz_2B_8k_n_fixed_256_tokenizer_1x15_32k
#SBATCH --output=eval_full_pile_steinmetz_2B_8k_n_fixed_256_tokenizer_1x15_32k_%A_%a.out
...
srun torchrun --nnodes $SLURM_NNODES --nproc_per_node 4 --max_restarts 1 --node_rank $SLURM_NODEID --rdzv_id 101 --rdzv_backend c10d --rdzv_endpoint "$MASTER_ADDR:$MASTER_PORT" ./evaluate.py --config ${CONFIG_FILE} --ckpt ${CKPT_PATH} --eval_steps 1000 --eval_dirname "eval-steinmetz" --source_dataset "steinmetz"
