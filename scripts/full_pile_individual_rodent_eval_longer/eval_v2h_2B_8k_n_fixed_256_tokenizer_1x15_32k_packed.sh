...
#SBATCH --time=6:00:00
#SBATCH --job-name=eval_full_pile_v2h_packed
#SBATCH --output=eval_full_pile_v2h_packed_%j.out   # %j gets replaced with the Job ID
...
for i in "${!CHECKPOINTS[@]}"; do
    ...
    echo "Starting v2h eval for $CKPT_PATH on nodes $SUB_NODELIST (Master: $MASTER_ADDR:$MASTER_PORT)"
    ...
    srun --nodelist=${SUB_NODELIST} \
         ...
         bash -c "torchrun --nnodes ${NODES_PER_JOB} \
                  ...
                  --ckpt ${CKPT_PATH} \
                  --eval_steps 1000 \
                  --eval_dirname \"eval-v2h\" \
                  --source_dataset \"v2h\"" \
         > "eval_v2h_log_${i}.out" 2>&1 &
...
wait
echo "All v2h evaluations complete."